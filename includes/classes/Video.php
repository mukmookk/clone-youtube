...
class Video {
    ...
    public function getLikes() {
        ...
        $query->bindParam(":videoId", $videoId);  // CALLING FUNCTION is an error!!
        $videoId = $this->getId();
        $query->execute();
    ...
    public function like() {
        ...
        $query->bindParam(":username", $username);          // THIS STATEMENT WILL EXECUTED WITH FUNCTION "execute()"
        $query->bindParam(":videoId", $id);

        $username = $this->userLoggedInObj->getUsername();  
        $query->execute();          

        // IF matched row exists
        if($query->rowCount() > 0) {
